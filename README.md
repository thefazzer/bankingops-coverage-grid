...
| **Public-eval surface overlay** — likelihood-matched projection of public rubrics/benchmarks/evals onto the SPEC-08 depth ladder; front-loaded LLMAJ (frozen observable prompt + prereg) for `public_benchmark`; division roll-up of saturation vs admitted voids | `specs/SPEC-09-public-eval-surface-overlay.md` | `reference/public-eval-inventory.v1.yaml` + `reference/public-eval-surface-map.v1.json` + `reference/public-eval-llmaj-prereg.v1.json` + `reference/public-eval-llmaj-ledger.v1.json` + schemas + `specs/prompts/public-eval-llmaj-judge.v1.txt` + `specs/rubrics/public-eval-mapping.yaml` | `python3 tools/public_eval_overlay.py check`; `python3 tools/public_eval_llmaj.py check` | S9 gates |
...
Rogo Big Finance Bench, Handshake BankerToolBench) are inventoried only;
`public_benchmark` paint requires front-loaded LLMAJ — an observable frozen
prompt and prereg pack (`reference/public-eval-llmaj-prereg.v1.json`) with
primary + sensitivity intersection-union — so neither relative public-eval
saturation nor seller own coverage can be steered. A derived division roll-up
reports saturation vs voids. Voids are admitted BOCG surface without painted
public-eval touch — not elicited competency gaps (I7). Presentation style is
left to consumers.
