...
| 19 | Public rubric/benchmark/eval density on the grid is disclosed as an explained overlay, not as elicited gaps or institution / own-cell coverage (BB construct validity; DS limitations; I7); public_benchmark paint is LLMAJ multi-pass (primary + sensitivity, intersection-union), never one-pass owner map | EVIDENCED | SPEC-09; `reference/public-eval-surface-map.v1.json` likelihood bands + paint-eligible inventory classes; `specs/rubrics/public-eval-mapping.yaml` + `tools/public_eval_llmaj.py` S9-G5; division_rollup voids vs saturation; S9 gates in `tools/gate_conformance.py` |
