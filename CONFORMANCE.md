...
| 19 | Public rubric/benchmark/eval density on the grid is disclosed as an explained overlay, not as elicited gaps or institution / own-cell coverage (BB construct validity; DS limitations; I7); public_benchmark paint is front-loaded LLMAJ (observable frozen prompt + prereg; primary + sensitivity; intersection-union), with no owner path to steer relative public-eval saturation or seller own coverage | EVIDENCED | SPEC-09; `reference/public-eval-llmaj-prereg.v1.json` + `specs/prompts/public-eval-llmaj-judge.v1.txt`; `tools/public_eval_llmaj.py` S9-G5; division_rollup voids vs saturation; S9 gates in `tools/gate_conformance.py` |
